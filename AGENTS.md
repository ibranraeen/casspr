# Casspr Development Guide

## Project Overview

Casspr is a terminal-based AI coding assistant built in Go by
...
The module path is `github.com/charmbracelet/casspr`.
...
internal/
  ...
  config/
    ...
    load.go                        casspr.json loading and validation
  ...
  hooks/                           Hook engine: runs user shell commands on hook events
    ...
    input.go                       Stdin payload builder, env vars, stdout parsing (Casspr + Claude Code compat)
...
- **Context files**: Casspr reads AGENTS.md, CASSPR.md, CLAUDE.md, GEMINI.md
...
- **Pub/sub**: `internal/pubsub` for decoupled communication between agent,
  UI, and services.
- **Hooks**: User-defined shell commands in `casspr.json` that fire before
