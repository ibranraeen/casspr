...
func (c *Client) SendMessage(ctx context.Context, id string, sessionID, runID, agentMode, prompt string, attachments ...message.Attachment) error {
...
// SubmitQuestion submits a response to a clarifying question.
func (c *Client) SubmitQuestion(ctx context.Context, id string, req proto.QuestionResponse) (bool, error) {
	rsp, err := c.post(ctx, fmt.Sprintf("/workspaces/%s/questions/submit", id), nil, jsonBody(req), http.Header{"Content-Type": []string{"application/json"}})
	if err != nil {
		return false, fmt.Errorf("failed to submit question response: %w", err)
	}
	defer rsp.Body.Close()
	if rsp.StatusCode != http.StatusOK {
		return false, fmt.Errorf("failed to submit question response: status code %d", rsp.StatusCode)
	}
	var resp proto.QuestionSubmitResponse
	if err := json.NewDecoder(rsp.Body).Decode(&resp); err != nil {
		return false, fmt.Errorf("failed to decode submit question response: %w", err)
	}
	return resp.Resolved, nil
}
