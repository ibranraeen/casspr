...
// CassprEnvMarkers returns a fresh slice of the environment variables that
// Casspr unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by Casspr.
	env = append(env, CassprEnvMarkers()...)
