...
// already in use by another casspr process.
var ErrDataDirLocked = errors.New("data directory already in use by another casspr process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to casspr.db so users can `ls` and find it.
const dataDirLockFile = "casspr.lock"
...
// {dataDir}/casspr.lock. If the lock is already held by another
...
// CASSPR_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("CASSPR_SKIP_DATADIR_LOCK"))
	return v
}
