...
import (
	...
	"github.com/ibranraeen/casspr/internal/format"
	...
	"github.com/ibranraeen/casspr/internal/ui/anim"
...
curl https://charm.land | casspr run "Summarize this website"

# Read from a file
casspr run "What is this code doing?" <<< prrr.go

# Redirect output to a file
casspr run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
casspr run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
casspr run --verbose "Generate a README for this project"

# Continue a previous session
casspr run --session {session-id} "Follow up on your last response"

# Continue the most recent session
casspr run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'casspr' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `casspr run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `casspr run` exited
...
// prefix returned by casspr session list.
