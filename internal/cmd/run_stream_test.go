...
// with reason=tool_use and used to terminate `casspr run` early via
...
// subscribers (e.g. a TUI watching workspace events while `casspr
...
// non-nil error from `casspr run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `casspr run` waits) exits cleanly rather
...
// queue scenario: `casspr run --continue` attaches to a session
