---
name: casspr-config
description: Use when the user needs help configuring Casspr — working with casspr.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Casspr behavior.
---

# Casspr Configuration

Casspr uses JSON configuration files with the following priority (highest to lowest):

1. `.casspr.json` (project-local, hidden)
2. `casspr.json` (project-local)
3. `$XDG_CONFIG_HOME/casspr/casspr.json` or `$HOME/.config/casspr/casspr.json` (global)

## Basic Structure

```json
{
  "$schema": "https://github.com/ibranraeen/casspr/casspr.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
Casspr runs selected string fields through an embedded bash-compatible
...
`casspr.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Casspr in a directory whose `casspr.json` you haven't
reviewed.
...
> `.agents/skills`, `.casspr/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".casspr/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".casspr/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `CASSPR_EVENT` | Event name (e.g. `PreToolUse`) |
| `CASSPR_TOOL_NAME` | Name of the tool being called |
| `CASSPR_SESSION_ID` | Current session ID |
| `CASSPR_CWD` | Current working directory |
| `CASSPR_PROJECT_DIR` | Project root directory |
| `CASSPR_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `CASSPR_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Casspr also supports the Claude Code hook output format:
...
- `CASSPR_GLOBAL_CONFIG` - Override global config location
- `CASSPR_GLOBAL_DATA` - Override data directory location
- `CASSPR_SKILLS_DIR` - Override default skills directory
