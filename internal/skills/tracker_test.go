...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (casspr-config).
	activeSkills := []*Skill{
		{Name: "casspr-config", Description: "Crush config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("casspr-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via casspr://...).
	tracker.MarkLoaded("casspr-config")
	require.True(t, tracker.IsLoaded("casspr-config"))
