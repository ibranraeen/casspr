...
func TestCassprIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .cassprignore file that ignores .log files
	require.NoError(t, os.WriteFile(".cassprignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .cassprignore file
	cassprignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".cassprignore"), []byte(cassprignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .cassprignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .cassprignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .cassprignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".cassprignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .cassprignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .cassprignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .cassprignore")
