...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["CASSPR_EVENT"])
	require.Equal(t, "bash", envMap["CASSPR_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["CASSPR_SESSION_ID"])
	require.Equal(t, "/work", envMap["CASSPR_CWD"])
	require.Equal(t, "/project", envMap["CASSPR_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["CASSPR_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["CASSPR_TOOL_INPUT_FILE_PATH"])
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$CASSPR_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
