...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	globalDir := t.TempDir()
	t.Setenv("CASSPR_GLOBAL_CONFIG", globalDir)
	t.Setenv("CASSPR_GLOBAL_DATA", globalDir)
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, ".casspr"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .casspr from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .casspr in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .casspr", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .casspr above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .casspr")
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "casspr_info", "casspr_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "glob", "ls", "sourcegraph", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "casspr_info", "casspr_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("CASSPR_DISABLE_DEFAULT_PROVIDERS", "true")
